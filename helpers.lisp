...
(defmethod scan ((entity sized-entity) (target vec2) on-hit)
  (let* ((loc (location entity))
         (size (bsize entity)))
    (when (and (let ((lx (vx2 loc))
                     (w (vx2 size)))
                 (<= (- lx w) (vx2 target) (+ lx w)))
               (let ((ly (vy2 loc))
                     (h (vy2 size)))
                 (<= (- ly h) (vy2 target) (+ ly h))))
      (let ((hit (make-hit entity loc)))
...
(defmethod scan ((entity sized-entity) (target sized-entity) on-hit)
  (let ((location (location target))
        (size (bsize target)))
   (scan entity (vec4 (vx2 location) (vy2 location) (vx2 size) (vy2 size))
         on-hit)))
...
(defmethod scan-collision (target (entity sized-entity))
  (let ((location (location entity))
        ...
        (best-time nil))
    (flet ((on-find (hit)
             (let ((object (hit-object hit)))
               (when (and (not (eql entity object))
                          (collides-p entity object hit))
                 (let ((dist (vsqrdist2 (hit-location hit) location))
                       (time (hit-time hit)))
